...
//P - An array, can be empty, else it is going to be an array of positive integers
//R - Number of minimum minutes it is going to take to make all orders on a single coffee machine
		...
		.forEach((el, index) => {
			if (coffees.length === 0) {
				...
				for (let i = 0; i <= index; i++) {
					minutes.push(coffees[i]);
				}
				minutes.push(2 * index);
			}
		});
...
function barista(coffees) {
	if (coffees.length === 0) {
		...
		let waitTime = sorted[0];
		let check = [sorted[0]];
		for (let i = 1; i < sorted.length; i++) {
			waitTime = waitTime + sorted[i] + 2;
			check.push(waitTime);
		}
		return check.reduce((acc, val) => acc + val);
	}
}
